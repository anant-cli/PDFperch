(function () {
  ...
  function setupHamburger() {
    ...
    document.addEventListener('click', function (e) {
      if (!headerContainer.contains(e.target) && nav.classList.contains('open')) closeMenu();
    });
    ...
    window.addEventListener('resize', function () {
      if (window.innerWidth > 800 && nav.classList.contains('open')) closeMenu();
    });
  ...
  function rewriteLocalFileLinks() {
    ...
    document.querySelectorAll('a[href^="/"]').forEach(function (anchor) {
      const normalizedHref = getNormalizedNavPath(anchor.getAttribute('href') || '');
  ...
  function setActiveNavLink() {
    ...
    document.querySelectorAll('.main-nav a').forEach(function (link) {
      ...
      const isActive = currentPath === href || (href === '/blog/blog_index' && currentPath.startsWith('/blog'));
      if (isActive) {
  ...
  function ensureSkipLink() {
    ensureMainId();
    if (document.querySelector('.skip-link') || !document.body) return;
  ...
  function normalizeGlobalLabels() {
    ...
    document.querySelectorAll('.back-btn').forEach(function (btn) {
      if (/back to home/i.test(btn.textContent)) btn.textContent = 'Back to Home';
    });
  }
  ...
  function initScrollReveals() {
    ...
    const observer = new IntersectionObserver(
      ...
      { threshold: CONSTANTS.INTERSECTION_THRESHOLD, rootMargin: '50px' }
    );
  ...
  function init3DTiltEffects() {
    document.querySelectorAll('.tool-card').forEach(function (card) {
      card.addEventListener('mousemove', function (e) {
        ...
        const rotateX = ((y - rect.height / 2) / rect.height) * -CONSTANTS.TILT_SENSITIVITY;
        const rotateY = ((x - rect.width / 2) / rect.width) * CONSTANTS.TILT_SENSITIVITY;
  ...
  function setupThemeToggle() {
    ...
    toggleBtn.addEventListener('click', function () {
      ...
      if (window.showToast) showToast(`Switched to ${nextTheme} mode!`, 'info');
    });

    const nav = headerContainer.querySelector('.main-nav');
    if (nav) headerContainer.insertBefore(toggleBtn, nav);
    else headerContainer.appendChild(toggleBtn);
  }

  function applyTheme(theme, toggleBtn) {
    document.documentElement.setAttribute('data-theme', theme);
    updateThemeMeta(theme);
    if (toggleBtn) updateToggleIcon(toggleBtn, theme);
  }
...
const RELATED_TOOLS_MAP = {
  'mergepdf':    ['compresspdf','signpdf','splitpdf','organizepdf'],
  'splitpdf':    ['mergepdf','rotatepdf','pagenumbers','organizepdf'],
  'compresspdf': ['mergepdf','pdfencrypt','pdf2word','watermarkpdf'],
  ...
  'rotatepdf':   ['mergepdf','splitpdf','organizepdf','pagenumbers'],
  'organizepdf': ['mergepdf','splitpdf','rotatepdf','pagenumbers'],
  'pagenumbers': ['organizepdf','mergepdf','watermarkpdf','compresspdf'],
  'pdf2jpg':     ['img2pdf','pdf2word','compresspdf','ocrtool'],
  'img2pdf':     ['pdf2jpg','mergepdf','compresspdf','docx2pdf'],
  'docx2pdf':    ['pdf2word','mergepdf','img2pdf','compresspdf'],
  'pdf2word':    ['docx2pdf','pdfencrypt','ocrtool','txt2docx'],
  'pptx2pdf':    ['docx2pdf','mergepdf','img2pdf','compresspdf'],
  'md2pdf':      ['docx2pdf','web2pdf','img2pdf','pagenumbers'],
  'web2pdf':     ['md2pdf','docx2pdf','compresspdf','mergepdf'],
  'txt2docx':    ['docx2pdf','ocrtool','md2pdf','mergepdf'],
  'ocrtool':     ['pdf2word','txt2docx','pdf2jpg','img2pdf'],
  'img2png':     ['imgcompress','img2pdf','pdf2jpg','compresspdf'],
  'imgcompress': ['img2png','img2pdf','compresspdf','pdf2jpg'],
  'qrmaker':     ['imgcompress','img2png','img2pdf','pdfencrypt'],
};

const TOOL_META = {
  'mergepdf':    { name:'Merge PDF',          icon:'JOIN', url:'/pages/mergepdf.html' },
  'splitpdf':    { name:'Split PDF',          icon:'CUT',  url:'/pages/splitpdf.html' },
  'compresspdf': { name:'Compress PDF',       icon:'ZIP',  url:'/pages/compresspdf.html' },
  'pdfencrypt':  { name:'Password Protect',   icon:'LOCK', url:'/pages/pdfencrypt.html' },
  'signpdf':     { name:'Sign PDF',           icon:'SIGN', url:'/pages/signpdf.html' },
  'watermarkpdf':{ name:'Watermark PDF',      icon:'MARK', url:'/pages/watermarkpdf.html' },
  'rotatepdf':   { name:'Rotate PDF',         icon:'90',   url:'/pages/rotatepdf.html' },
  'organizepdf': { name:'Organize PDF',       icon:'ORG',  url:'/pages/organizepdf.html' },
  'pagenumbers': { name:'Page Numbers',       icon:'#',    url:'/pages/pagenumbers.html' },
  'pdf2jpg':     { name:'PDF to JPG',         icon:'JPG',  url:'/pages/pdf2jpg.html' },
  'img2pdf':     { name:'Images to PDF',      icon:'IMG',  url:'/pages/img2pdf.html' },
  'docx2pdf':    { name:'Word to PDF',        icon:'DOCX', url:'/pages/docx2pdf.html' },
  'pdf2word':    { name:'PDF to Word',        icon:'DOC',  url:'/pages/pdf2word.html' },
  'pptx2pdf':    { name:'PPTX to PDF',        icon:'PPT',  url:'/pages/pptx2pdf.html' },
  'md2pdf':      { name:'Markdown to PDF',    icon:'MD',   url:'/pages/md2pdf.html' },
  'web2pdf':     { name:'HTML to PDF',        icon:'HTML', url:'/pages/web2pdf.html' },
  'txt2docx':    { name:'TXT to Word',        icon:'TXT',  url:'/pages/txt2docx.html' },
  'ocrtool':     { name:'OCR Text Extract',   icon:'OCR',  url:'/pages/ocrtool.html' },
  'img2png':     { name:'Image Converter',    icon:'IMG',  url:'/pages/img2png.html' },
  'imgcompress': { name:'Compress Images',    icon:'IMG',  url:'/pages/imgcompress.html' },
...
function renderRelatedTools() {
  ...
  if (document.getElementById('relatedToolsSection')) return;
